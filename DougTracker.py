...
# Constants
###########################################################################
XPOSITION       = 12 # DISTANCE OF CMAERA FROM CENTER OF ROBOT
...
FOV_PERPIXEL = 0.064 
FOV_PERPIXEL_RAD = FOV_PERPIXEL * math.pi/180
CAM_ANGLE = 0
...
#    boundingCorners = []    
    ...
    if len(minAreaCorners) > 1:
     ...
     #        distance = distance * math.cos(CAM_ANGLE) - ZPOSITION
             print(angleToTarget)
             imageText ="Angle:"+str(int(angleToTarget * 10)/10.0) +" Distance:"+str(int(distance *10)/10.0)
